...
exports.addExpense = async (req, res) => {
  try {
    ...
    validator.notNull(expense.expenseMembers)) {
      ...
      if (!ownerValidation) {
        const err = new Error("Please provide valid group owner");
        err.status = 400;
        throw err;
      }
      for (let user of expense.expenseMembers) {
        const memberValidation = await validator.groupUserValidation(
          user,
          expense.groupId
        );
        if (!memberValidation) {
          const err = new Error(
            "Please ensure the members exists in the group"
          );
          err.status = 400;
          throw err;
        }
      ...
      expense.currency = group.groupCurrency;

      // new expense model from req.body
      ...
      const update_response = await groupDAO.addSplit(
        expense.groupId,
...
exports.recentUserExpenses = async (req, res) => {
    try {
        ...
        if (recentExpense.length == 0) {
            var err = new Error("No recent expense present for the user")
            err.status = 400
            throw err
        }
        res.status(200).json({
